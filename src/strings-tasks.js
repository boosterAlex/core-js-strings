...
function removeLastOccurrences(str, value) {
  ...
  return str.substring(0, index) + str.substring(value.length + index);
}
...
function endsWith(str, substr) {
  return str.trim().endsWith(substr);
}
...
function reverseWords(str) {
  ...
  words.forEach((word) => {
    reversWords.push(word.split('').reverse().join(''));
  });
...
function invertCase(str) {
  ...
  const invertChar = characters.map((char) => {
    if (char === char.toUpperCase()) return char.toLowerCase();
    if (char !== char.toUpperCase()) return char.toUpperCase();
    return char;
  });
  return invertChar.join('');
}
